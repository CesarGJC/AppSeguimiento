...
public class OperacionesSqlProvider {
    public static String getOperaciones(Map<String, Object> params) {
        FilterRequest<FormularioFilterRequest> filtro = (FilterRequest<FormularioFilterRequest>) params.get("filtro");
        StringBuilder sql = new StringBuilder("SELECT * FROM public.vw_operaciones WHERE  1=1");
    ...
    public static String getCantidadOperaciones(Map<String, Object> params) {
        FilterRequest<FormularioFilterRequest> filtro = (FilterRequest<FormularioFilterRequest>) params.get("filtro");
        StringBuilder sql = new StringBuilder("SELECT count(id_formulario) FROM public.vw_operaciones WHERE  1=1");
        for (var item : filtro.getParams()) {
